...
var processfile = function(file, checks) {
    ...
    console.log(outJson);
    fs.writeFile('submit.json', outJson);
};

var assertURLExists = function(url, resultfile, checks) {
    var response = function(result, response) {
	if (result instanceof Error) {
	    console.log('Error: ' + util.format(response.message));
	} else {
	    resultfile = resultfile || FILE_READIN;
	    fs.writeFileSync(resultfile, result);
	    var instr = assertFileExists(resultfile);
	    processfile(instr, checks);
	}
    };
...
if(require.main == module) {
	...
	.option('-u, --url <url>', 'URL of file')
        .parse(process.argv);
    if (program.url) {
	assertURLExists(program.url, program.file, program.checks);
    } else {
